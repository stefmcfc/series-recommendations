...
| `GET` | `/api/v1/series/search` | Search and filter series (`title`, `genre`, `keyword`, `status`, rating ranges, `startedNotFinished`). `sortBy`/`sortDirection` — see below. |
