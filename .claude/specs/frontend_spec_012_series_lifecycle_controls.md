...
- Requirement 5's rewatch toggle UI uses a visible `<label>` ("Rewatch" on `SeriesList` rows, "Flag for rewatch" on `SeriesDetail`) with `aria-label="Flag for rewatch"` on the `<input>` so `getByLabelText(/flag for rewatch/i)` resolves consistently in both components, per the sketches in this spec. **As of the 2026-08-27 amendment below, both the visible text and `aria-label` become dynamic (state-dependent) rather than static** — the initial unflagged-state text described here is unchanged, but see that amendment for the flagged-state text.
...
**Amendment (2026-08-27)**: the rewatch toggle's label text and `aria-label` were static regardless of `flaggedForRewatch` state (parts 2/3 above only changed its visual treatment, filled vs. outline), leaving the flagged/unflagged distinction signaled by color alone — a real ambiguity for a user who can't rely on color (and a WCAG "use of color" concern, not just a UX one), flagged during a live review of `tooling_spec_006`'s otherwise-unrelated refactor work. No new acceptance criteria — this extends AC-12/AC-13's existing toggle behavior, which never pinned exact button text, rather than introducing new requirement scope. Both the visible text and `aria-label` now swap together based on `flaggedForRewatch`, keeping them in sync (WCAG 2.5.3 Label in Name): `SeriesDetail` reads "Flag for rewatch" ↔ "Flagged for rewatch"; `SeriesList` reads "Rewatch" ↔ "Flagged" (kept short for the row's tighter layout), with `aria-label` using the full "Flag for rewatch"/"Flagged for rewatch" phrasing in both components for a consistent accessible name. Files touched: `frontend/src/components/SeriesDetail.tsx`, `frontend/src/components/SeriesList.tsx`. Test files touched: `SeriesDetail.test.tsx` and `SeriesList.test.tsx` — the existing `FRONTEND-012-AC-13`/`AC-12` toggle-success and revert-on-failure tests were each extended with `toHaveTextContent`/`aria-label` assertions for both the flagged and reverted-to-unflagged states, rather than adding new test blocks, since these are extensions of the existing toggle tests' coverage, not new scenarios. Verified: `npm test` (496/496) and `npm run lint` both clean; confirmed live via Claude in Chrome (both components, both directions) that the button text swaps correctly and no console errors are logged.
