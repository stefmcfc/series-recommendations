...
**Amendment (2026-08-24, live review)**: the List page's inline field's default (empty-input) suggestion list
read as cluttered in that field's narrower layout — good on the Recommendations page, not here. Reverted just
that field's `maxSuggestionsWhenEmpty` from `KEYWORD_SUGGESTIONS_LIMIT` to `0` (no suggestions until typed,
`frontend_spec_029`'s original behavior), since "Browse all keywords" already covers browsing the full list
without typing (AC-05/AC-10, unaffected). `allowFreeText`/typed-filter behavior on the inline field are
unchanged. `SearchFilter.test.tsx`'s AC-04-adjacent test updated to assert the reverted behavior.
**Priority**: Medium
