...
Two real gaps in `SeriesService`, found while scoping a frontend validation request (`frontend_spec_091`) and confirmed by reading the service directly. First: `applyCurrentSeason` rejects a `currentSeason` update that exceeds `totalSeasons`, but the equivalent `currentEpisode`/`totalEpisodes` pair has no check at all — `update()` sets `currentEpisode` unconditionally inside `applyMetadataUpdates`. This asymmetry was a **deliberate** decision at the time (`frontend_spec_004`'s Design Decisions: "the backend doesn't do that check either, so adding one client-side would create a false rejection the server wouldn't agree with") — this spec closes it on the backend first, so the frontend fix land on top of matching server behavior rather than reintroducing that same mismatch in the other direction. Second: `rottenTomatoesRating`/`rottenTomatoesPopcornmeter` had no **imperative, service-level** range check — confirmed via grep across `SeriesDto`/`SeriesService` (no `@Min`/`@Max` annotation on the DTO, no imperative bounds check in the service). **Correction, found during implementation**: `SeriesEntity` (not checked in the original research pass, which only looked at `SeriesDto`) already carries `@Min(0)`/`@Max(100)` bean-validation annotations on both fields — an out-of-range value was never silently persisted, but it surfaced as a raw `ConstraintViolationException` wrapped in a `TransactionSystemException` at `repository.save()` time, deep inside a `@Transactional` method, not a clean `IllegalArgumentException` the way `title`/`imdbRating`/`year`/`currentSeason` already get. The fix below still stands and is still worthwhile — it converts an ugly, generic transaction-failure exception into the same clean, early, controlled 400 every other validation failure in this service produces — but the original "silently persists e.g. `rottenTomatoesRating: 500`" framing was wrong and is corrected here rather than left standing in a spec now marked Complete.
