...
- **FRONTEND-027-AC-07** [AUTO] — **superseded by `frontend_spec_080`, 2026-09-03**: `minSourceRating` (`FRONTEND-011-AC-07`) shall be hidden under both `Popular Right Now` and `Highest Rated`, extending the same mode-based visibility rule already applied to `Genre & Keyword`. **`minSourceRating` (and its whole `showMinSourceRating` visibility gate) is retired entirely as of `frontend_spec_080`** — there is nothing left to show or hide, so this AC's own test was removed rather than left asserting against deleted UI. This ID is preserved per this project's immutable-reference-ID rule, not deleted.
