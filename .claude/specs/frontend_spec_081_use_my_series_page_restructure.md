...
**Amendment (2026-09-03, live review, PR #157 still open)**: reworked Section 1's internal layout before merge, in place of the original AC-01/AC-03 wrapping. The Genre/Status/Sort row's `.specificSeriesFiltersRow`/`.specificSeriesRightColumn` two-column arrangement (added 2026-08-27 as an explicitly temporary, no-spec layout tweak — see the removed comment in `UseMySeriesPanel.tsx`) is retired entirely. New top-to-bottom order inside the disclosure body: **Filter by Status** (own full-width row), **Sort by** (own full-width row, directly below Status), **Genre | Keyword** (a fixed 4-column grid row, each field spanning 2 columns), **Min Personal Rating | Min IMDb Rating | Min TMDB Rating (My Series)** (a fixed 4-column grid row, each field spanning 1 column, 4th column empty), **Year Min (My Series) | Year Max (My Series)** (same fixed 4-column grid, 1 column each, 3rd/4th empty). New CSS: `.filterFullWidthRow` (`grid-column: 1 / -1`, reused by Status/Sort by), `.filterFourColGrid` (`grid-column: 1 / -1; display: grid; grid-template-columns: repeat(4, minmax(0, 1fr))`, deliberately fixed-track rather than `auto-fit` so the narrow rating/year fields don't stretch to fill the row — same convention as `SeriesDetail.module.css`'s `.threeColRow`), `.filterSpanTwo` (`grid-column: span 2`). Two behavior changes landed alongside the layout: (1) the Keywords field (`specific-series-keywords`) no longer accepts free text — `allowFreeText` removed, since this field only ever narrows against a tracked series' actual keywords, so an untracked typed value could never match anything; `SearchFilter.tsx`'s own Keywords field and `RecommendationFiltersBox`'s Exclude Keywords field are unaffected, both still allow free text for their own valid reasons. (2) Min Personal Rating's `<span>Min Personal Rating</span>` label (unstyled because `StarRating` has no single input to `htmlFor`-associate a real `<label>` with) now picks up the same bold/heading-color treatment as its sibling fields' real `<label>`s, via `RecommendationControls.module.css`'s `.field label, .field > span` selector (direct-child combinator, not descendant, to avoid also styling unrelated `<span>`s nested inside `KeywordPicker`/`GenreIncludeExcludePicker`'s own internal markup within the same `.field` wrapper). The identical unstyled-span pattern in `SearchFilter.tsx`/`SearchFilter.module.css` was left untouched — out of scope for this amendment. No AC text changed; ACs 01/03/04 remain satisfied by the new structure (disclosure/toggle mechanics unchanged, Genre/Status/Sort filtering logic unchanged, Keywords filtering logic unchanged aside from the free-text removal).
**Priority**: P2
