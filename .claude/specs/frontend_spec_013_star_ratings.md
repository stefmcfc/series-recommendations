...
Rating field (Requirement 3's target, `FRONTEND-013-AC-07`/`AC-08`) now renders via the shared
`SeriesFormFields.tsx` component, not directly in either form file — implement the `<StarRating>`
swap there once. `SeriesDetail`/`SeriesList`'s Personal Rating displays (Requirement 2) are
unaffected by this and still live directly in those two components.
