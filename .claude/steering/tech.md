...
## API Documentation
Wired up via `org.springdoc:springdoc-openapi-starter-webmvc-ui:3.1.1` in `backend/build.gradle.kts` (`series_spec_066`). No further configuration is required for the dependency itself — adding it alone makes `/swagger-ui.html` (interactive UI), `/v3/api-docs` (JSON spec), and `/v3/api-docs.yaml` (YAML spec) available against the running app, generated live from the actual controller/DTO code across all 9 controllers. `config/OpenApiConfig.java` supplies the spec's title ("TV Series Tracker API") and description; its `version` is bound to `org.springframework.boot.info.BuildProperties` (auto-configured by Spring Boot's `ProjectInfoAutoConfiguration` from `META-INF/build-info.properties`, which the `springBoot { buildInfo() }` block in `build.gradle.kts` generates at build time), so it always matches `build.gradle.kts`'s `version` field rather than being hand-maintained separately. Per-parameter `@Operation`/`@Parameter` descriptions are a separate, not-yet-built pass (`series_spec_067`).

## CI/CD
GitHub Actions (`.github/workflows/ci.yml`, `codeql.yml`) runs on every push to `main` and every PR — backend (`gradle check` + `gradle build -x test`), frontend (lint, format check, test, build, audit), and a gitleaks secrets scan. CodeQL (java + javascript-typescript) runs on PRs and a weekly schedule, not on push to `main` — a squash-merge lands the exact tree already analyzed in the PR. A `changes` job in each workflow gates backend/frontend/CodeQL's per-language matrix leg on whether that half of the repo actually changed, and separately skips all of them outright on a release-cut-only diff (just `CHANGELOG.md` + the two version fields) — since a release cut bumps both version fields on nearly every merge regardless of which side changed, a version-line-only edit to `build.gradle.kts`/`package.json` doesn't by itself count as that side changing. See `.claude/steering/ci_cd_pipeline.md` for the full pipeline diagram and `RUNBOOK.md`'s CI/CD and Troubleshooting sections for the specific commands and the dependency-version pitfalls that have broken it before (Gradle wrapper/CI version drift, `eslint-plugin-jsx-a11y` vs `eslint 10`, `hibernate-community-dialects` pinning).
...
- **Date/time "now" calls take an injected `Clock`, not an implicit default.** A single `Clock` bean (`config/ClockConfig.java`, `Clock.systemDefaultZone()`) is available for constructor injection — call `LocalDateTime.now(clock)`/`Year.now(clock)`/etc rather than the no-arg overload, which implicitly uses the JVM's default time zone (flagged by SonarQube `java:S8688`). Established across `SeriesService`, `SeriesRefreshService`, `BulkRefreshService`, `IgnoredSeriesService`, `RecommendationCriteriaValidator`. In a Spock spec, construct the class under test with `Clock.systemDefaultZone()` directly (see any of the above services' `*Spec.groovy` for the pattern) rather than mocking `Clock` unless a test genuinely needs a fixed instant.
