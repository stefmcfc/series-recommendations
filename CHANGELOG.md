...
- Rotten Tomatoes Popcornmeter UI: `AddSeriesForm` and `EditSeriesForm` gain a "Rotten Tomatoes Rating (Popcornmeter)" numeric input (0–100, same validation/payload-omit-when-empty convention as the existing field, which is now labeled "Rotten Tomatoes Rating (Tomatometer)" to disambiguate); it's not wired into the TMDB-lookup autofill path since the backend lookup result doesn't return it either. `SeriesDetail`'s Ratings section now shows both Rotten Tomatoes scores as a `96%`-style percentage (`—` when unset) in a 3-field row alongside Personal Rating (`frontend_spec_037`).
