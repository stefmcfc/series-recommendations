...
- Backend: the generated OpenAPI spec's title/description identify this app, and its version is bound to `BuildProperties` (`springBoot { buildInfo() }`) so it always matches `build.gradle.kts`'s real version rather than a hardcoded or missing value (`series_spec_066`).
...
- Frontend: My Series' and Use My Series' Country/Language filters now respect saved favourites (`countryFavourites`/`languageFavourites`), pinning them to the top of the picker like every other Country/Language field in the app — the initial implementation used a option list meant only for the Settings favourites editor, with no favourites pinning at all.
...
- Frontend: My Series and Recs > Use My Series show a count message when series are hidden from a rating sort for missing that rating, and both gain two new sort options, "Tomatometer" and "Popcornmeter" — with My Series' rating column showing the value as a percent with its emoji (🍅/🍿) when sorted by either (`frontend_spec_119`).
...
- Backend: `JsonNodeConverter` now fails with a clear `IllegalStateException` on malformed persisted `criteria` JSON instead of an unhandled crash (`series_spec_056`).
...
- Frontend: `/settings`' Refresh All skip-threshold override is now a number input plus a Days/Weeks/Months unit `<select>` (default Days, no Hours/Minutes — a single-series refresh already bypasses the threshold entirely), replacing the previous bare "(minutes)"-labeled number field; a blank field still sends no override at all regardless of the selected unit (`frontend_spec_101` FRONTEND-101-AC-01/03).
- Frontend: `SettingsSection` gains an optional `icon?: ReactNode` prop, rendered immediately before the title with `aria-hidden="true"` so it never affects the section's accessible name (`frontend_spec_101` FRONTEND-101-AC-05/08).
- Frontend: new `components/SettingsIcons.tsx` — five hand-rolled inline SVG icons (Appearance, Refresh, Export, Import, Favourites), stroke-based with `currentColor` so they theme automatically in light/dark with no extra CSS; no new npm dependency (`frontend_spec_101` FRONTEND-101-AC-07).
...
- CI: `backend`/`frontend` (and CodeQL's per-language `analyze` matrix leg) now also skip when that half of the repo didn't change at all — a docs-only or frontend-only PR no longer runs a `gradle check`, and vice versa. A version-line-only bump to `backend/build.gradle.kts`/`frontend/package.json` doesn't count as a real change on its own, since nearly every merge touches both as part of its own release cut regardless of which side the feature is on.
- Fix: CodeQL's per-language skip is now a step-level `if:` inside the `analyze` job instead of a job-level one — referencing `matrix.language` in a job-level `if:` makes GitHub reject the whole workflow file as invalid, which meant `analyze` was silently not running on any trigger at all (caught while verifying the change above on PR #174).
- CI: CodeQL (`codeql.yml`) no longer runs on push to `main` — only on PRs and the existing weekly schedule — since a squash-merge lands the exact tree its PR run already analyzed.
- CI: `ci.yml` and `codeql.yml` each gain a `concurrency` group so a superseded PR run is cancelled when a new commit is pushed; pushes to `main` are never cancelled.
- Docs: `CLAUDE.md`'s Git workflow section now explicitly discourages cutting a release as a separate follow-up `chore(release)` branch/PR — it must be a commit on the feature branch itself, before merging.
...
- Frontend: `GenreIncludeExcludePicker` now renders removable chips (one per included/excluded genre, each with its own "×") below its trigger button whenever a selection exists, in both `includeExclude` and `excludeOnly` modes — letting a user remove a single genre without reopening the picker's modal; also renamed the shared button/dialog-heading label from "Genres"/"Filter by Genre" to "Include / Exclude Genres" at its 3 include/exclude call sites (`SearchFilter`, `UseMySeriesPanel`, `CustomSearchPanel`), leaving `RecommendationFiltersBox`'s exclude-only "Exclude Genres" label unchanged (`frontend_spec_076`).
...
- Frontend: `KeywordPicker` gains an optional `hideInput` prop that suppresses its text input (and label), leaving its suggestions list (including any default empty-input list) and selected-pills list (still removable) unaffected (`frontend_spec_077`).
