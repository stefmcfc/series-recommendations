...
| `app.tmdb.watch-region` | `GB` | Region consulted by `WatchProviderService` when resolving each `GET /api/v1/series/recommendations` candidate's, or `GET /api/v1/series/{id}/watch-providers` series', currently-available `flatrate` (subscription-streaming) watch providers (`TmdbClient.watchProviders`, `GET /tv/{tmdbId}/watch/providers`) -- a single configured value, not a per-request parameter (single-user personal app, one household's viewing region). Resolution is best-effort and non-fatal: a failed or empty lookup just yields an empty `streamingProviders` list on that one candidate/series. Overridable via `APP_TMDB_WATCH_REGION`. See `series_spec_020_watch_providers.md`. |
