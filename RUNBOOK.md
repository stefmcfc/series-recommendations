...
| `app.tmdb.max-per-source` | `8` | `RecommendationService`'s per-source diversity cap -- how many recommendations can come from any single source series before lower-ranked excess candidates from it are dropped -- see `series_spec_007_recommendation_sourcing.md` (`SERIES-007-AC-22`). Overridable via `APP_TMDB_MAX_PER_SOURCE`. |
