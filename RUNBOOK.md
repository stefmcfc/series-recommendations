...
| `app.tmdb.refresh-skip-threshold-minutes` | `60` | During a bulk refresh only, a series whose `lastRefreshedAt` is within this many minutes of now is skipped (still counted toward `completedCount`, tracked separately via `skippedCount`) rather than re-fetched -- `0` disables skipping entirely. Single-series refresh (`POST /api/v1/series/{id}/refresh`) never applies this threshold. Overridable via `APP_TMDB_REFRESH_SKIP_THRESHOLD_MINUTES`. See `series_spec_018_series_refresh.md`. |
