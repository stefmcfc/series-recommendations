...
- `app.tmdb.api-key` gates `GET /api/v1/series/recommendations`, `GET /api/v1/series/recommendations/{tmdbId}/keywords`, `GET /api/v1/series/lookup/search-tmdb`, `GET /api/v1/series/lookup/resolve-tmdb`, and `GET /api/v1/series/{id}/watch-providers` (`/recommendations` specifically only fails once there's at least one `COMPLETED` series with a resolvable `imdbId` to source from; with none yet, it returns `200` with an empty list regardless of whether the TMDB key is set). Unlike the others, `/recommendations/{tmdbId}/keywords` and `/{id}/watch-providers` never fail with `502` even when the key is unset or the TMDB call fails -- per `series_spec_023_recommendation_metadata_and_overview.md` and `series_spec_026_series_watch_providers.md` respectively, both always return `200` (an empty list, or an empty `streamingProviders` list), since there's no persisted entity here for a `502` to be meaningful against; `/{id}/watch-providers` still `404`s for a genuinely unknown series `id`, which is unrelated to the TMDB key. It's also consulted (same best-effort, non-fatal posture) by `POST /api/v1/series/{id}/refresh` and the bulk `POST`/`GET /api/v1/series/refresh-all` job (`series_spec_018_series_refresh.md`) -- an unset/failing key there just leaves `title`/`year`/`genres`/`totalSeasons`/`totalEpisodes`/`tmdbRating`/`tmdbVoteCount`/`productionStatus`/`originCountry`/`overview`/`keywords` unchanged and `tmdbRefreshed: false` on the result, never fails the refresh (single or bulk) (`keywords` reconciliation added by `series_spec_019_keyword_tracking.md`; `overview` added by `series_spec_023_recommendation_metadata_and_overview.md`; `title`/`year`/`genres` added by `series_spec_040_tmdb_managed_field_lock.md`, which also has these same three fields -- plus `totalSeasons`/`totalEpisodes`/`imdbRating` -- locked from manual `PATCH` edit once non-null, refresh being the only way to change them after that point). `GET /api/v1/series/keywords` never calls TMDB itself -- it only aggregates already-stored keyword data -- so it works the same whether or not the key is set.
...
| `app.tmdb.max-source-series` | `20` | Upper bound on how many source series (the automatic `COMPLETED` pool, or an explicit `seriesIds` selection) feed title-based recommendation sourcing -- see `series_spec_007_recommendation_sourcing.md`. Overridable via `APP_TMDB_MAX_SOURCE_SERIES`. |
| `app.tmdb.max-candidates` | `50` | Upper bound on the combined raw recommendation candidate pool before `external_ids` is resolved for each one -- see `series_spec_007_recommendation_sourcing.md`. Overridable via `APP_TMDB_MAX_CANDIDATES`. |
| `app.tmdb.max-discover-pages` | `6` | Upper bound on how many additional TMDB pages `sourceTrending`/`sourceTopRated`/`sourceByGenreOrKeyword`'s backfill-pagination loop may fetch for a single `GET /api/v1/series/recommendations` request, once a page's results still leave that sourcing mode short of the requested `limit` after dedup/output-filtering -- does not apply to "Use My Series" (`sourceFromPool`) sourcing -- see `series_spec_054_recommendation_discover_backfill_pagination.md`. Overridable via `APP_TMDB_MAX_DISCOVER_PAGES`. |
| `app.tmdb.max-per-source` | `8` | `RecommendationRankingService`'s per-source diversity cap -- how many recommendations can come from any single source series before lower-ranked excess candidates from it are dropped -- see `series_spec_007_recommendation_sourcing.md` (`SERIES-007-AC-22`). Overridable via `APP_TMDB_MAX_PER_SOURCE`. |
| `app.tmdb.refresh-delay-ms` | `250` | Fixed delay (milliseconds) between items during a bulk refresh (`POST /api/v1/series/refresh-all`) or a bulk import (`POST /api/v1/series/import`), sized to stay well under TMDB's free-tier rate limit (~40 requests/10s) for a personal collection's realistic size -- see `series_spec_018_series_refresh.md` and `series_spec_038_import.md` (which reuses this property rather than adding its own). Overridable via `APP_TMDB_REFRESH_DELAY_MS`. |
| `app.tmdb.refresh-skip-threshold-minutes` | `60` | During a bulk refresh only, a series whose `lastRefreshedAt` is within this many minutes of now is skipped (still counted toward `completedCount`, tracked separately via `skippedCount`) rather than re-fetched -- `0` disables skipping entirely. Single-series refresh (`POST /api/v1/series/{id}/refresh`) never applies this threshold. Overridable via `APP_TMDB_REFRESH_SKIP_THRESHOLD_MINUTES`. `POST /api/v1/series/refresh-all` also accepts a `skipThresholdMinutesOverride` in its request body to override this value for a single run only, without changing this configured default for future runs; `GET /api/v1/series/refresh-all/status`'s `skipThresholdMinutesUsed` reports whichever value actually governed a given run. See `series_spec_018_series_refresh.md` and `series_spec_052_refresh_skip_threshold_override.md`. |
| `app.tmdb.watch-region` | `GB` | Default region consulted by `WatchProviderService` when resolving each `GET /api/v1/series/recommendations` candidate's, or `GET /api/v1/series/{id}/watch-providers` series', currently-available `flatrate` (subscription-streaming) watch providers (`TmdbClient.watchProviders`, `GET /tv/{tmdbId}/watch/providers`) -- governs whenever a request omits the optional per-request `region` override both endpoints now accept (`series_spec_053_watch_region_override.md`; the frontend always resolves and sends one via its own persisted Settings value, `frontend_spec_102_settings_watch_region.md`). Resolution is best-effort and non-fatal: a failed or empty lookup just yields an empty `streamingProviders` list on that one candidate/series. Overridable via `APP_TMDB_WATCH_REGION`. See `series_spec_020_watch_providers.md`. |
| `app.recommendations.pool-cache-ttl-minutes` | `10` | How long `RecommendationPoolCache` keeps a "Use My Series" sourced candidate pool (`RecommendationSourcingService.sourceFromPool`'s raw TMDB output, keyed on `seriesIds`/`limit`) before treating it as stale and re-sourcing from TMDB -- so a `sortBy`-only (or other post-sourcing-only) re-request within the window is served from cache instead of re-hitting TMDB. Overridable via `APP_RECOMMENDATIONS_POOL_CACHE_TTL_MINUTES`. See `series_spec_035_use_my_series_pool_cache.md` and `series_spec_045_retire_min_source_rating.md` (which removed `minSourceRating` from the cache key). |
| `app.recommendations.pool-cache-max-entries` | `50` | Capacity bound on `RecommendationPoolCache` -- once full, already-expired entries are swept first, then (if still full) the single oldest remaining entry is evicted to make room. Overridable via `APP_RECOMMENDATIONS_POOL_CACHE_MAX_ENTRIES`. See `series_spec_035_use_my_series_pool_cache.md`. |
| `app.recommendations.diversity-cap-mode` | `best-source` | `RecommendationRankingService`'s per-source diversity cap mode -- controls which candidates are kept when a single source series contributes more than `app.tmdb.max-per-source` recommendations. Overridable via `APP_RECOMMENDATIONS_DIVERSITY_CAP_MODE`. |
| `app.tmdb.default-min-vote-count` | `200` | Default floor on a candidate's TMDB vote count before it's eligible as a recommendation, used by `RecommendationSourcingService`/`RecommendationOutputFilterService`/`RecommendationDefaults` when the request doesn't specify its own `minVoteCount` -- `0` disables the filter entirely. Overridable via `APP_TMDB_DEFAULT_MIN_VOTE_COUNT`. See `series_spec_029_configurable_min_vote_count_and_genre_floor.md`. |
...
| `VITE_KEYWORD_SUGGESTIONS_LIMIT` | `10` | Caps how many tracked keywords `KeywordPicker` shows as default suggestions while its input is empty, on the Recommendations page's Discover > `Custom Search` field (formerly labeled "Genre & Keyword" -- see `frontend_spec_042_recommendation_source_mode_reorganization.md`) and the List page's inline `Keywords` filter field (`src/utils/keywordSuggestions.ts`). Undefined or non-numeric values fall back to the default. Does not apply to the List page's "Browse all keywords" modal, which always shows every tracked keyword uncapped -- see `frontend_spec_032_hybrid_keyword_suggestions.md`. |
| `VITE_SERIES_DETAIL_SECTION_HEADERS` | `true` | Temporary toggle for `SeriesDetail`'s "Details"/"Ratings"/"Personal"/"Timeline" section headers above its grouped field rows, while deciding whether they read better than the plain grouped rows. Set to `false` to hide them (any other value, or unset, shows them). Takes effect on the next page reload -- see `frontend_spec_012_series_lifecycle_controls.md`'s live-review part-3 amendment. |
...
GitHub Actions runs on every push to `main` and every PR (`.github/workflows/ci.yml`, plus `codeql.yml` for security scanning — CodeQL's own trigger is PRs + a weekly schedule, not push to `main`; see below):

- **changes** (`ci.yml` and `codeql.yml`, each has its own copy): diffs the incoming commit(s) and sets three outputs: `backend`/`frontend` (did that half of the repo, or this workflow file, actually change — see below for why this isn't plain path-matching) and `release_cut_only` (is the diff exactly `CHANGELOG.md` + the version line in `backend/build.gradle.kts` + the version line in `frontend/package.json`, the shape of a release-cut commit, e.g. `385859e`). `backend` only runs when its output is `true`; `frontend` likewise; CodeQL's `analyze` matrix gates its `java` leg on `backend` and its `javascript-typescript` leg on `frontend`. `release_cut_only=true` forces all of them to skip regardless. Because nearly every merge bumps both version fields as part of its own release cut (`CLAUDE.md`'s "Versioning & changelog" section), a version-line-only change to `build.gradle.kts`/`package.json` doesn't by itself count as "backend changed"/"frontend changed" — only an additional real change elsewhere under `backend/**`/`frontend/**` does, otherwise every merge would trip both regardless of which side it actually touched. See `.claude/steering/ci_cd_pipeline.md` for the full rationale and diagram.
...
- **changelog-cut**: push-to-`main` only. Fails the build if `[Unreleased]` in `CHANGELOG.md` is still non-empty once something has landed on `main` — see `CLAUDE.md`'s Git workflow section.
- **secrets-scan**: gitleaks over the full checkout.
- **CodeQL analyze** (`codeql.yml`, java + javascript-typescript matrix): runs on PRs and a weekly Monday 06:00 UTC cron only — no push-to-`main` trigger, since a squash-merge lands the exact tree its PR run already analyzed, and the weekly cron catches drift between merges. Each matrix leg is additionally gated per-language by the `changes` job's `backend`/`frontend` outputs.
